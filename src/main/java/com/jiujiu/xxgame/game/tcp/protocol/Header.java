 ...
 */
public class Header {
	private final static Logger logger = LogManager.getLogger(Header.class);
	/** 协议类型 */
		...
		for (Field f : fs) {
			...
			if (!Modifier.isStatic(mod)) {
				if (type == "byte") {
					...
					logger.warn("过滤了非基础数据类型属性：" + Modifier.toString(mod) + " - " + type + "[" + f.getName() + "]");
				}
			} else {
				if (!"SIZE".equals(f.getName())) {
					logger.warn("过滤了静态属性：" + Modifier.toString(mod) + " - " + type + "[" + f.getName() + "]");
				}
			}
		}
		bits /= 8;
		if (bits > Byte.MAX_VALUE || bits < Byte.MIN_VALUE) {
			logger.error("数值[" + bits + "]超过byte边界");
			return 0;
		}
		return bits;
	}
