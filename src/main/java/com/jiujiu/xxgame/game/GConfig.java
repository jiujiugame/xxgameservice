package com.jiujiu.xxgame.game;

import com.jiujiu.xxgame.game.utils.FileUtil;
import com.jiujiu.xxgame.game.utils.PropertiesUtil;
...
public final class GConfig {
	private final static Logger logger = LogManager.getLogger(GConfig.class);
	...
	public synchronized void init(String fileName) {
		if (!inited) {
			...
			logger.info("已加载系统参数 -> {}", configs.size());
		}
	}
	...
	private void load(File configFile) {
		...
		for (Map.Entry<String, String> entry : confs.entrySet()) {
			if (!configs.containsKey(entry.getKey())) {
				...
				logger.warn("{} -> 配置重复,game.properties中的配置无效!", entry.getKey());
			}
		}
	}
